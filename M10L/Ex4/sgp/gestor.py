...
class Gestor:
    ...
    def adicionar_administrador(self):
        try:
            ...
            print("Erro: Código do setor deve ser um número inteiro e valores de salário, impostos e ajuda de custo devem ser números.")

    def remover_administrador(self):
        nome = input("Nome do administrador a ser removido: ")
        administrador = self.buscar_administrador(nome)
        if administrador:
            self.administradores.remove(administrador)
            print("Administrador removido com sucesso!")
        else:
            print("Administrador não encontrado.")

    def atualizar_administrador(self):
        nome = input("Nome do administrador a ser atualizado: ")
        administrador = self.buscar_administrador(nome)
        if administrador:
            try:
                novo_endereco = input("Novo endereço: ")
                ...
                novo_base_salary = float(input("Novo Salário Base: "))
                novo_tax = float(input("Novos Impostos (%): "))
                novo_subsistence_allowance = float(input("Nova Ajuda de Custo: "))
                administrador.set_endereco(novo_endereco)
                administrador.set_cpf(novo_cpf)
                administrador.set_rg(novo_rg)
                administrador.set_telefone(novo_telefone)
                administrador.set_sector_code(novo_sector_code)
                administrador.set_base_salary(novo_base_salary)
                administrador.set_tax(novo_tax)
                administrador.set_subsistence_allowance(novo_subsistence_allowance)
                print("Informações atualizadas com sucesso!")
            except ValueError:
                print("Erro: Código do setor deve ser um número inteiro e valores de salário, impostos e ajuda de custo devem ser números.")
        else:
    ...
    def buscar_administrador(self, nome):
        for administrador in self.administradores:
            if administrador.get_nome() == nome:
